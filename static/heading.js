function draw_heading_canvasAz(degrees, ctx, options, drawNeedle) {
...
function draw_heading_canvasEl(degrees, ctx, options, drawNeedle) {
    //draw a compass like canvas to indicate antenna heading
    ...
    if (drawLegend){
        ctx.font = font;
        ctx.fillStyle = color;
        ctx.fillText("180",16*options.scale, (cy/2) + 5);
        ctx.fillText("90", cx/2-11, 25*options.scale);
        ctx.fillText("0", cx-21*options.scale, cy/2 + 5);
        ctx.fillStyle = "red";
        ctx.font = font;
    }
    ...
    if (drawNeedle){
        //compass needle
        ctx.translate(cx/2,cy/2);
		ctx.rotate(-1*(degrees -90)* Math.PI / 180 + Math.PI);
        ctx.beginPath();
        ctx.moveTo(-6,0);
        ctx.lineTo(0,r);
        ctx.lineTo(6,0);
